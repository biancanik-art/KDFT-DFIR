...
use crate::{
    ...
    ndb::{
        block::{
            read_to_end_bounded, DataTree, IntermediateTreeBlock, MAX_MATERIALIZED_NODE_BYTES,
        },
        block_id::BlockId,
        header::Header,
    ...
    <Pst as PstFile>::Store: StoreReadWrite<Pst>,
{
    fn read(store: Rc<Pst::Store>) -> io::Result<Self> {
        ...
        let data = read_to_end_bounded(&mut reader, MAX_MATERIALIZED_NODE_BYTES as usize)?;
        let size = data.len();
