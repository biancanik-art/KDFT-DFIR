...
const MAX_HEAP_TREE_LEVELS: u8 = 16;
const MAX_HEAP_TREE_RECORDS: usize = 1_000_000;
...
impl HeapTreeHeader {
    pub fn new(key_size: u8, entry_size: u8, levels: u8, root: HeapId) -> LtpResult<Self> {
        ...
        if levels > MAX_HEAP_TREE_LEVELS {
            return Err(LtpError::InvalidHeapTreeLevelCount(levels));
        }
    ...
    V: HeapTreeEntryValue + HeapNodePageReadWrite,
{
    ...
    fn entries(&self) -> io::Result<Vec<HeapTreeLeafEntry<K, V>>> {
        ...
        let mut visited = BTreeSet::from([u32::from(header.root())]);
        ...
        while level > 0 {
            for heap_id in mem::take(&mut next_level).into_iter() {
                ...
                while let Ok(row) = HeapTreeIntermediateEntry::<K>::read(&mut cursor) {
                    let next = row.next_level();
                    let raw = u32::from(next);
                    if !visited.insert(raw) {
                        return Err(LtpError::DuplicateHeapTreeNode(raw).into());
                    }
                    if visited.len() > MAX_HEAP_TREE_RECORDS {
                        return Err(LtpError::HeapTreeRecordLimitExceeded.into());
                    }
        ...
        for heap_id in mem::take(&mut next_level).into_iter() {
            ...
            while let Ok(row) = HeapTreeLeafEntry::<K, V>::read(&mut cursor) {
                if results.len() >= MAX_HEAP_TREE_RECORDS {
                    return Err(LtpError::HeapTreeRecordLimitExceeded.into());
                }
                results.push(row);
            }
