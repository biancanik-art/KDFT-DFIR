...
const MAX_BLOCK_GROUPS: u64 = 1_048_576;
    ...
    R: ReadAt,
{
    ...
    // These fields live in the extended portion of the fixed-size
    // superblock, beyond the fields consumed sequentially below.
    let s_checksum_type = entire_superblock[0x175];
    let s_checksum_seed = read_le32(&entire_superblock[0x270..0x274]);
    ...
    let _s_feature_compat = inner.read_u32::<LittleEndian>()?; /* compatible feature set */
    ...
    let unsupported_layout = IncompatibleFeature::JOURNAL_DEV
        | IncompatibleFeature::META_BG
        | IncompatibleFeature::DIRDATA;
    ensure!(
        !incompatible_features.intersects(unsupported_layout),
        unsupported_feature(format!(
            "unsupported ext4 layout feature(s): {:?}",
            incompatible_features & unsupported_layout
        ))
    );

    // Feature support is validated at the level where it affects decoding.
    // Unknown feature bits are rejected above; recognized data-affecting
    // features are rejected for individual inodes when their bytes are read.
    ...
    if has_checksums {
            ...
            s_checksum_type == 1,
            unsupported_feature(format!(
                "unsupported ext4 metadata checksum type {s_checksum_type}"
            ))
        );
    ...
    if 0 == s_blocks_per_group {
        return Err(parse_error("blocks per group cannot be zero".to_string()));
    }
    ...
    let block_size: u32 = match s_log_block_size {
        ...
        s_inode_size >= INODE_BASE_LEN as u16
            && u32::from(s_inode_size) <= block_size
            && s_inode_size.is_power_of_two(),
        assumption_failed(format!(
            "invalid inode size {s_inode_size} for {block_size}-byte blocks"
        ))
    );
    let bitmap_capacity = block_size
        .checked_mul(8)
        .ok_or_else(|| assumption_failed("ext4 bitmap capacity overflow"))?;
    ensure!(
        s_blocks_per_group <= bitmap_capacity,
        assumption_failed(format!(
            "blocks per group {s_blocks_per_group} exceed bitmap capacity {bitmap_capacity}"
        ))
    );
    ensure!(
        s_inodes_per_group <= bitmap_capacity,
        assumption_failed(format!(
            "inodes per group {s_inodes_per_group} exceed bitmap capacity {bitmap_capacity}"
        ))
    );
    let expected_first_data_block = u32::from(block_size == 1024);
    ensure!(
        s_first_data_block == expected_first_data_block,
        assumption_failed(format!(
            "invalid first data block {s_first_data_block} for {block_size}-byte blocks"
        ))
    );
    ...
    if long_structs {
            ...
            s_desc_size >= 64,
            assumption_failed(format!(
                "64-bit ext4 block group descriptors must be at least 64 bytes, not {}",
                s_desc_size
            ))
        );
    } else {
        ensure!(
            s_desc_size == 0 || s_desc_size == 32,
            assumption_failed(format!(
                "legacy ext4 block group descriptor size must be 0 or 32, not {}",
                s_desc_size
            ))
        );
    }
    ...
    let data_blocks = total_blocks
        ...
        .ok_or_else(|| assumption_failed("ext4 block count precedes first data block"))?;
    let blocks_per_group = u64::from(s_blocks_per_group);
    let group_count = data_blocks
        .checked_add(blocks_per_group - 1)
        .ok_or_else(|| assumption_failed("ext4 block group count overflow"))?
        / blocks_per_group;
    ensure!(
        group_count <= MAX_BLOCK_GROUPS,
        unsupported_feature(format!(
            "ext4 block group count {group_count} exceeds safe parser limit {MAX_BLOCK_GROUPS}"
        ))
    );
    ...
    F: FnOnce(u64) -> Result<Vec<u8>, Error>,
{
    ...
    if let Some(uuid_checksum) = uuid_checksum {
        ...
        let computed = ext4_style_crc32c_le(
            checksum_prefix.ok_or_else(|| assumption_failed("missing inode checksum prefix"))?,
            &data,
        );
        ...
        if let Some(high) = i_checksum_hi {
            ...
            let short_computed = (computed & 0xFFFF) as u16;
            ensure!(
...
fn read_xattrs(
    ...
    loop {
        ...
        let e_value_size = read_le32(&reading[0x08..0x0C]);
            ...
            e_block == 0,
            unsupported_feature("external xattr value inodes are not supported")
        );
        ...
        let end = start
            .checked_add(usize::try_from(e_value_size)?)
            .ok_or_else(|| assumption_failed("xattr value range overflow"))?;
        ...
        let next_record = end_of_name
            .checked_add((4 - (end_of_name % 4)) % 4)
            .ok_or_else(|| assumption_failed("xattr record offset overflow"))?;
        ensure!(
            next_record <= reading.len(),
            assumption_failed("xattr record extends beyond block")
        );
...
mod tests {
    ...
    fn synthetic_metadata_csum_image(checksum_type: u8, checksum_seed: u32) -> Vec<u8> {
        let mut image = vec![0u8; 4096];
        let sb = &mut image[1024..2048];
        sb[0x04..0x08].copy_from_slice(&2u32.to_le_bytes()); // blocks count
        sb[0x14..0x18].copy_from_slice(&1u32.to_le_bytes()); // first data block
        sb[0x20..0x24].copy_from_slice(&8u32.to_le_bytes()); // blocks per group
        sb[0x28..0x2c].copy_from_slice(&4u32.to_le_bytes()); // inodes per group
        sb[0x38..0x3a].copy_from_slice(&super::EXT4_SUPER_MAGIC.to_le_bytes());
        sb[0x3a..0x3c].copy_from_slice(&1u16.to_le_bytes()); // clean state
        sb[0x4c..0x50].copy_from_slice(&1u32.to_le_bytes()); // dynamic revision
        sb[0x58..0x5a].copy_from_slice(&128u16.to_le_bytes());
        sb[0x60..0x64].copy_from_slice(&(0x0040u32 | 0x2000u32).to_le_bytes());
        sb[0x64..0x68].copy_from_slice(&0x0400u32.to_le_bytes()); // metadata_csum
        sb[0x68..0x78].copy_from_slice(b"different-uuid!!");
        sb[0xfe..0x100].copy_from_slice(&32u16.to_le_bytes());
        sb[0x175] = checksum_type;
        sb[0x270..0x274].copy_from_slice(&checksum_seed.to_le_bytes());

        let checksum = ext4_style_crc32c_le(!0, &sb[..1020]);
        sb[1020..1024].copy_from_slice(&checksum.to_le_bytes());

        let descriptor = &mut image[2048..2080];
        descriptor[8..12].copy_from_slice(&3u32.to_le_bytes());
        let prefix = ext4_style_crc32c_le(checksum_seed, &0u32.to_le_bytes());
        let descriptor_checksum = ext4_style_crc32c_le(prefix, descriptor) as u16;
        descriptor[0x1e..0x20].copy_from_slice(&descriptor_checksum.to_le_bytes());
        image
    }
    ...
    #[test]
    fn checksum_seed_feature_validates_group_descriptors_with_stored_seed() {
        let image = synthetic_metadata_csum_image(1, 0x1234_5678);
        super::superblock(image.as_slice(), &crate::Options::default()).unwrap();
    }

    #[test]
    fn unsupported_metadata_checksum_algorithm_is_rejected() {
        let image = synthetic_metadata_csum_image(2, 0x1234_5678);
        assert!(super::superblock(image.as_slice(), &crate::Options::default()).is_err());
    }
