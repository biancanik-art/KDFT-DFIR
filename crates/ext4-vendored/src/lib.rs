#![forbid(unsafe_code)]
// Vendored from crates.io `ext4` 0.9.0 (FauxFaux/ext4-rs, MIT licensed - see
// LICENSE.MIT) because the upstream `SuperBlock::new`'s incompatible-feature
// allowlist rejects `CSUM_SEED`, a feature flag modern e2fsprogs/mkfs.ext4
// sets by default (confirmed against a real Kali Linux VM disk image). That
// flag only affects how metadata checksums are seeded; it does not change
// extent/directory-entry layout, so it does not affect this crate's ability
// to read data correctly - rejecting it outright made this crate unable to
// open essentially any modern real-world ext4 filesystem. KDFT also exposes
// read-only inode/first-extent location metadata used to place examiner byte
// views at authoritative decoded-media offsets. The crate has been
// unmaintained upstream since 2021.
/*!
...
/// Location of the first real bytes that back an inode in the filesystem
/// byte stream. `filesystem_offset` is relative to the start of the ext
/// volume; callers that opened ext inside a disk image add the partition
/// start to obtain a decoded-media offset.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DataLocation {
    pub filesystem_offset: u64,
    pub file_offset: u64,
    pub contiguous_bytes: u64,
    pub storage: DataLocationStorage,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DataLocationStorage {
    Extent,
    InlineSymlink,
}
    ...
    R: ReadAt,
{
    ...
    /// Byte offset of the inode record itself, relative to the filesystem.
    pub fn inode_disk_offset(&self, inode: u32) -> Result<u64, Error> {
        self.groups.index_of(inode)
    }

    /// Resolves the first byte range that physically backs an inode.
    ///
    /// Short symlink targets live inline in the inode's i_block area. Regular
    /// files and long symlinks use extents; the returned file offset can be
    /// non-zero when the file starts with a sparse range.
    pub fn first_data_location(&self, inode: &Inode) -> Result<Option<DataLocation>, Error> {
        if inode.stat.size == 0 {
            return Ok(None);
        }
        if matches!(inode.stat.extracted_type, FileType::SymbolicLink)
            && inode.stat.size < u64::try_from(INODE_CORE_SIZE)?
        {
            let filesystem_offset = self
                .inode_disk_offset(inode.number)?
                .checked_add(0x28)
                .ok_or_else(|| assumption_failed("inline symlink byte offset overflow"))?;
            return Ok(Some(DataLocation {
                filesystem_offset,
                file_offset: 0,
                contiguous_bytes: inode.stat.size,
                storage: DataLocationStorage::InlineSymlink,
            }));
        }

        let reader = inode.reader(&self.inner)?;
        Ok(reader.first_extent_location()?.map(
            |(file_offset, filesystem_offset, contiguous_bytes)| DataLocation {
                filesystem_offset,
                file_offset,
                contiguous_bytes,
                storage: DataLocationStorage::Extent,
            },
        ))
    }
    ...
    {
        self.walk_inner(inode, path, visit, &mut HashSet::new(), 0)
    }
    ...
    fn walk_inner<F>(
        ...
        path: &str,
        visit: &mut F,
        active_directories: &mut HashSet<u32>,
        depth: usize,
    ) -> Result<bool, Error>
    where
        F: FnMut(&Self, &str, &Inode, &Enhanced) -> Result<bool, Error>,
    {
        const MAX_DIRECTORY_DEPTH: usize = 256;
            ...
            assumption_failed(format!(
                "directory nesting exceeds safe limit of {MAX_DIRECTORY_DEPTH}"
            ))
        );
        ...
        if let Enhanced::Directory(entries) = enhanced {
                ...
                active_directories.insert(inode.number),
                assumption_failed(format!(
                    "directory cycle detected at inode {} ({path})",
                    inode.number
                ))
            );
    ...
    pub fn resolve_path(&self, path: &str) -> Result<DirEntry, Error> {
        ...
        let Some(last) = parts.pop() else {
            return Err(parse_error(
                "path did not contain a final component".to_string(),
            ));
        };
        for part in parts {
            if part.is_empty() {
    ...
    /// Read the data from an inode. Callers should normally use this only for regular files.
    ...
    R: ReadAt,
{
    let offset = block
        ...
        .ok_or_else(|| assumption_failed("ext4 block byte offset overflow"))?;
...
impl Inode {
        ...
        R: ReadAt,
    {
        ...
        loop {
                ...
                read <= total_len && total_len - read >= 8,
                assumption_failed("truncated ext4 directory record header")
            );
            ...
            let minimum_record_len = 8usize
                .checked_add(usize::from(name_len))
                .ok_or_else(|| assumption_failed("directory record length overflow"))?;
            ensure!(
                usize::from(rec_len) >= minimum_record_len,
                assumption_failed(format!(
                    "directory record length {rec_len} is shorter than its {name_len}-byte name"
                ))
            );
            let record_end = read
                .checked_add(usize::from(rec_len))
                .ok_or_else(|| assumption_failed("directory record offset overflow"))?;
            ensure!(
                record_end <= total_len,
                assumption_failed(format!(
                    "directory record extends beyond inode data: {record_end} > {total_len}"
                ))
            );
