...
impl HeapFillLevel {
    fn unpack_fill_levels(value: u32) -> Result<[HeapFillLevel; 8], LtpError> {
        Ok([
            HeapFillLevel::try_from((value & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 4) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 8) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 12) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 16) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 20) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 24) & 0x0F) as u8)?,
            HeapFillLevel::try_from(((value >> 28) & 0x0F) as u8)?,
        ])
    }
    ...
    <Pst as PstFile>::DataBlock: BlockReadWrite + Clone,
{
        ...
        <Pst as PstFile>::DataBlock: 'a,
    {
        ...
        checked_heap_allocation(block, alloc)
    }
}

fn checked_heap_allocation<'a>(block: &'a [u8], alloc: &HeapNodePageAlloc) -> io::Result<&'a [u8]> {
    let start = alloc.offset() as usize;
    let end = start
        .checked_add(alloc.size() as usize)
        .ok_or(LtpError::InvalidHeapPageAllocOffset(alloc.offset()))?;
    block
        .get(start..end)
        .ok_or_else(|| LtpError::InvalidHeapPageAllocOffset(alloc.offset()).into())
}
...
mod hardening_tests {
    ...
    #[test]
    fn heap_allocation_outside_block_is_rejected_without_panicking() {
        let block = [0_u8; 16];
        let alloc = HeapNodePageAlloc {
            offset: 12,
            size: 8,
        };
        let error = checked_heap_allocation(&block, &alloc).unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::InvalidData);
    }

    #[test]
    fn heap_allocation_at_block_boundary_is_accepted() {
        let block = [0_u8; 16];
        let alloc = HeapNodePageAlloc {
            offset: 12,
            size: 4,
        };
        assert_eq!(checked_heap_allocation(&block, &alloc).unwrap().len(), 4);
    }
}
