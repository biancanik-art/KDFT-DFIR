...
mod tests {
    ...
    fn parse_volume_typical() {
        ...
        assert_eq!(vol.chunk_size().unwrap(), 32768); // 64 * 512
        assert_eq!(vol.total_size().unwrap(), 512 * 64000);
    }
    ...
    #[test]
    fn ewf_reader_parallel_read_at_preserves_chunk_order() {
        let data1 = vec![0x31; 32768];
        let data2 = vec![0x72; 32768];
        let tmp = build_synthetic_e01_two_chunks(&data1, &data2);
        let reader = EwfReader::open(tmp.path()).unwrap();

        // A read of at least 64 KiB spanning more than one chunk takes the
        // bounded Rayon path in `read_at`. The returned evidence bytes must
        // remain in logical image order regardless of worker completion order.
        let mut actual = vec![0u8; 65536];
        let read = reader.read_at(&mut actual, 0).unwrap();
        assert_eq!(read, actual.len());
        assert_eq!(&actual[..32768], data1.as_slice());
        assert_eq!(&actual[32768..], data2.as_slice());
    }
    ...
    fn parse_ewf2_device_info_short_data() {
        ...
        let mut ts = 0u64;
        // Empty data
        parse_ewf2_device_info(&[], &mut cs, &mut ts).unwrap();
        assert_eq!(cs, 0);
        // 1 byte (too short for UTF-16)
        parse_ewf2_device_info(&[0x41], &mut cs, &mut ts).unwrap();
        assert_eq!(cs, 0);
    }
    ...
    fn parse_ewf2_device_info_too_few_lines() {
        ...
        parse_ewf2_device_info(&utf16, &mut cs, &mut ts).unwrap();
        assert_eq!(cs, 0);
    }
    ...
    #[test]
    fn validate_reorder_rejects_mismatched_file_and_header_counts() {
        let (_dir, files) = make_temp_files(2);
        let result = crate::reader::validate_and_reorder_segments(files, vec![1]);
        assert!(result.unwrap_err().to_string().contains("parsed 1 segment"));
    }
