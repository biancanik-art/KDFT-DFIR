...
/// Maximum size of a materialized MAPI property. The bound prevents corrupt
/// property metadata from exhausting memory; callers receive `InvalidData`
/// and can report partial coverage.
pub const MAX_PROPERTY_VALUE_BYTES: usize = 64 * 1024 * 1024;
const MAX_MULTI_VALUE_COUNT: usize = 1_000_000;

fn checked_multi_value_count(count: u32) -> LtpResult<usize> {
    let count = count as usize;
    if count > MAX_MULTI_VALUE_COUNT {
        return Err(LtpError::InvalidMultiValuePropertyCount(count));
    }
    Ok(count)
}

pub(crate) fn read_property_bytes_bounded<R: Read + ?Sized>(
    reader: &mut R,
    buffer: &mut Vec<u8>,
) -> io::Result<()> {
    read_property_bytes_bounded_with_limit(reader, buffer, MAX_PROPERTY_VALUE_BYTES)
}

fn read_property_bytes_bounded_with_limit<R: Read + ?Sized>(
    reader: &mut R,
    buffer: &mut Vec<u8>,
    max_bytes: usize,
) -> io::Result<()> {
    // Read one byte beyond the accepted maximum so an exact-boundary value is
    // distinguishable from a larger stream without trusting corrupt length
    // metadata or consuming the remainder of that stream.
    let probe_limit = max_bytes.checked_add(1).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "PST property materialization bound cannot be usize::MAX",
        )
    })?;
    buffer.clear();
    reader.take(probe_limit as u64).read_to_end(buffer)?;
    if buffer.len() > max_bytes {
        buffer.clear();
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "PST property exceeds the {} byte materialization safety bound",
                max_bytes
            ),
        ));
    }
    ...
    fn read(f: &mut dyn Read, prop_type: PropertyType) -> io::Result<Self> {
        match prop_type {
            ...
            PropertyType::String8 => {
                ...
                read_property_bytes_bounded(f, &mut buffer)?;
            ...
            PropertyType::MultipleString8 => {
                // ulCount
                let count = checked_multi_value_count(f.read_u32::<LittleEndian>()?)?;
                ...
                let mut start = offsets
                    ...
                    .ok_or(LtpError::InvalidMultiValuePropertyCount(offsets.len()))?;
                ...
                for i in 0..offsets.len() {
                    ...
                    let mut buffer = if i < offsets.len() - 1 {
                        ...
                        if next < start || (next - start) % mem::size_of::<u16>() != 0 {
                            return Err(LtpError::InvalidMultiValuePropertyOffset(next).into());
                        }

                        if next > start {
                            let length = next - start;
                            if length > MAX_PROPERTY_VALUE_BYTES {
                                return Err(LtpError::InvalidMultiValuePropertyOffset(next).into());
                            }
        ...
        match value.value() {
            ...
            PropertyValueRecord::Node(sub_node_id) => {
                ...
                let result = data_tree
                    ...
                    .and_then(|mut reader| read_property_bytes_bounded(&mut reader, &mut data));
...
#[cfg(test)]
mod bounded_property_reader_tests {
    use super::*;

    const TEST_LIMIT: usize = 16;

    struct EndlessByteReader {
        bytes_read: usize,
    }

    impl Read for EndlessByteReader {
        fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
            buffer.fill(0xA5);
            self.bytes_read = self.bytes_read.saturating_add(buffer.len());
            Ok(buffer.len())
        }
    }

    #[test]
    fn bounded_reader_accepts_value_at_exact_limit() {
        let expected = vec![0x5A; TEST_LIMIT];
        let mut reader = Cursor::new(expected.clone());
        let mut output = vec![0xFF; 4];

        read_property_bytes_bounded_with_limit(&mut reader, &mut output, TEST_LIMIT).unwrap();

        assert_eq!(output, expected);
        assert_eq!(reader.position(), TEST_LIMIT as u64);
    }

    #[test]
    fn bounded_reader_rejects_limit_plus_one_and_discards_partial_value() {
        let mut reader = Cursor::new(vec![0x5A; TEST_LIMIT + 1]);
        let mut output = Vec::new();

        let error = read_property_bytes_bounded_with_limit(&mut reader, &mut output, TEST_LIMIT)
            .unwrap_err();

        assert_eq!(error.kind(), io::ErrorKind::InvalidData);
        assert!(error.to_string().contains(&TEST_LIMIT.to_string()));
        assert!(output.is_empty());
        assert_eq!(reader.position(), (TEST_LIMIT + 1) as u64);
    }

    #[test]
    fn bounded_reader_never_consumes_an_unbounded_stream() {
        let mut reader = EndlessByteReader { bytes_read: 0 };
        let mut output = Vec::new();
        ...
        assert_eq!(error.kind(), io::ErrorKind::InvalidData);
        assert_eq!(reader.bytes_read, TEST_LIMIT + 1);
        assert!(output.is_empty());
    }

    #[test]
    fn enormous_multi_value_count_is_rejected_before_allocation() {
        let mut cursor = Cursor::new(u32::MAX.to_le_bytes());
        let error = PropertyValue::read(&mut cursor, PropertyType::MultipleGuid).unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::InvalidData);
    }
}
