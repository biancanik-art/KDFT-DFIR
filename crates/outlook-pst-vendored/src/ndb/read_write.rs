    ...
    <Self as Block>::Trailer: BlockTrailerReadWrite,
{
    ...
    fn read<R: PstReader>(f: &mut R, size: u16, encoding: NdbCryptMethod) -> io::Result<Self> {
        ...
        let allocated_size = checked_block_size(size, Self::Trailer::SIZE)?;
        let mut data = vec![0; size as usize];
        f.read_exact(&mut data)?;

        let occupied_size = size
            .checked_add(Self::Trailer::SIZE)
            .ok_or(NdbError::InvalidBlockSize(size))?;
        let offset = i64::from(allocated_size - occupied_size);
        if offset > 0 {
    ...
    fn write<W: Write + Seek>(&self, f: &mut W) -> io::Result<()> {
        ...
        let data_size = u16::try_from(data.len())
            .map_err(|_| NdbError::InvalidBlockAllocationSize(u32::MAX))?;
        let allocated_size = checked_block_size(data_size, Self::Trailer::SIZE)?;
        let trailer = Self::Trailer::new(data_size, trailer.signature(), crc, trailer.block_id())?;

        f.write_all(&data)?;

        let occupied_size = data_size
            .checked_add(Self::Trailer::SIZE)
            .ok_or(NdbError::InvalidBlockSize(data_size))?;
        let offset = i64::from(allocated_size - occupied_size);
        if offset > 0 {
            f.seek(SeekFrom::Current(offset))?;
        }
    ...
    <Self as IntermediateTreeBlock>::Trailer: BlockTrailerReadWrite,
{
    ...
    fn read<R: PstReader>(f: &mut R, header: Self::Header, size: u16) -> io::Result<Self> {
        ...
        let mut data = vec![0; size as usize];
        f.read_exact(&mut data)?;
        let mut cursor = Cursor::new(&data[usize::from(header_size)..usize::from(required_size)]);
    ...
    fn write<W: Write + Seek>(&self, f: &mut W) -> io::Result<()> {
        let entries_size = self
            .entries()
            .len()
            .checked_mul(usize::from(Self::Entry::ENTRY_SIZE))
            .ok_or(NdbError::InvalidInternalBlockEntryCount(u16::MAX))?;
        let data_len = usize::from(Self::Header::HEADER_SIZE)
            .checked_add(entries_size)
            .ok_or(NdbError::InvalidInternalBlockEntryCount(u16::MAX))?;
        let data_size =
            u16::try_from(data_len).map_err(|_| NdbError::InvalidBlockAllocationSize(u32::MAX))?;
        let allocated_size = checked_block_size(data_size, Self::Trailer::SIZE)?;
        let mut curor = Cursor::new(vec![0_u8; data_len]);
        ...
        let occupied_size = data_size
            .checked_add(Self::Trailer::SIZE)
            .ok_or(NdbError::InvalidBlockSize(data_size))?;
        let offset = allocated_size - occupied_size;
