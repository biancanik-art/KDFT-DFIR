...
pub enum NdbError {
    ...
    #[error("Invalid allocated block size: 0x{0:X}")]
    InvalidBlockAllocationSize(u32),
    ...
    #[error("PST tree traversal exceeded the maximum safe depth")]
    TreeTraversalDepthExceeded,
    #[error("PST node data exceeds the safe materialization limit: {0} bytes")]
    NodeDataTooLarge(u64),
    #[error("PST data tree exceeds the safe block-count limit")]
    DataTreeBlockLimitExceeded,
}
