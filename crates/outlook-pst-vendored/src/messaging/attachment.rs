...
use crate::{
    ltp::{
        heap::HeapNode,
        prop_context::{read_property_bytes_bounded, BinaryValue, PropertyContext, PropertyValue},
        ...
        let (properties, data) = {
            ...
            let data = match attachment_method {
                ...
                AttachmentMethod::Storage => {
                    ...
                    let mut data = vec![];
                    let mut data_block_cache = Default::default();
                    let mut reader = block.reader(
                        file,
                        encoding,
                        &block_btree,
                        &mut page_cache,
                        &mut data_block_cache,
                    )?;
                    read_property_bytes_bounded(&mut reader, &mut data)?;
