...
pub enum LtpError {
    ...
    #[error("Invalid BTHHEADER bIdxLevels: 0x{0:02X}")]
    InvalidHeapTreeLevelCount(u8),
    #[error("BTH traversal contains a cycle or duplicate HID: 0x{0:08X}")]
    DuplicateHeapTreeNode(u32),
    #[error("BTH traversal exceeds the safe record-count limit")]
    HeapTreeRecordLimitExceeded,
}
