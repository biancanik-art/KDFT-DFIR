...
use crate::{
    ...
    ltp::{
        heap::HeapNode,
        prop_context::{GuidValue, PropertyContext, PropertyValue, MAX_PROPERTY_VALUE_BYTES},
...
impl NamedPropReadWrite for StringEntry {
    fn read(f: &mut dyn Read) -> io::Result<Self> {
        ...
        if size % 2 != 0 || size as usize > MAX_PROPERTY_VALUE_BYTES {
            return Err(
    ...
    pub fn lookup_guid(&self, index: NamedPropertyGuid) -> io::Result<GuidValue> {
        ...
        match stream_guid {
            PropertyValue::Binary(value) => {
                ...
                let end = start.checked_add(16).ok_or(
                    MessagingError::NamedPropertyMapGuidIndexOutOfBounds(u16::from(index)),
                )?;
                let bytes = value.buffer().get(start..end).ok_or(
                    MessagingError::NamedPropertyMapGuidIndexOutOfBounds(u16::from(index)),
                )?;
        ...
        match stream_string {
            PropertyValue::Binary(value) => {
                ...
                while let Ok(value) = StringEntry::read(&mut cursor) {
                    let padding = (4 - i64::from(value.size()) % 4) % 4;
                    results.push(value);
...
mod hardening_tests {
    ...
    fn properties(id: u16, value: PropertyValue) -> NamedPropertyMapProperties {
        NamedPropertyMapProperties {
            properties: BTreeMap::from([(id, value)]),
        }
    }

    #[test]
    fn zero_bucket_count_is_rejected_before_modulo() {
        let map = properties(0x0001, PropertyValue::Integer32(0));
        assert!(map.bucket_count().is_err());
    }

    #[test]
    fn out_of_range_guid_and_string_offsets_are_rejected() {
        let map = properties(
            0x0002,
            PropertyValue::Binary(BinaryValue::new(vec![0_u8; 4])),
        );
        assert!(map.lookup_guid(NamedPropertyGuid::GuidIndex(2)).is_err());

        let map = properties(
            0x0004,
            PropertyValue::Binary(BinaryValue::new(vec![0_u8; 4])),
        );
        assert!(map.lookup_string(5).is_err());
    }

    #[test]
    fn string_stream_padding_skips_only_alignment_bytes() {
        let mut data = Vec::new();
        for ch in [b'A' as u16, b'B' as u16] {
            data.extend_from_slice(&2_u32.to_le_bytes());
            data.extend_from_slice(&ch.to_le_bytes());
            data.extend_from_slice(&[0_u8; 2]);
        }
        let map = properties(0x0004, PropertyValue::Binary(BinaryValue::new(data)));
        let strings = map.stream_string().unwrap();
        assert_eq!(strings.len(), 2);
        assert_eq!(strings[0].to_string(), "A");
        assert_eq!(strings[1].to_string(), "B");
    }
}
