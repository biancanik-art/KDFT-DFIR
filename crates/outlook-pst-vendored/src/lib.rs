...
pub enum PstError {
    ...
    #[error("Invalid declared PST length: declared {declared} bytes, actual {actual} bytes")]
    InvalidDeclaredFileSize { declared: u64, actual: u64 },
}
        ...
        IntermediateTreeEntryReadWrite,
{
    fn read_from(mut reader: Box<dyn PstReader>) -> io::Result<Self> {
        ...
        let declared_size: u64 = root.file_eof_index().index().into();
        if declared_size > actual_size || declared_size < PAGE_SIZE as u64 {
            return Err(PstError::InvalidDeclaredFileSize {
                declared: declared_size,
                actual: actual_size,
            }
            .into());
        }
        for page in [root.node_btree().index(), root.block_btree().index()] {
            let offset: u64 = page.index().into();
            let end = offset
                .checked_add(PAGE_SIZE as u64)
                .ok_or(PstError::InvalidBTreePage(offset))?;
            if offset % PAGE_SIZE as u64 != 0 || end > declared_size || end > actual_size {
                return Err(PstError::InvalidBTreePage(offset).into());
            }
        }
        let density_list =
    ...
    fn read_block(&self, block: <Pst as PstFile>::BlockId) -> io::Result<Vec<u8>> {
        ...
        block.ensure_materializable()?;
        let mut block_cache = Default::default();
        let mut block_reader = block.reader(
            reader,
            encoding,
            &block_btree,
            &mut page_cache,
        ...
        read_to_end_bounded(&mut block_reader, MAX_MATERIALIZED_NODE_BYTES as usize)
    }
}

pub fn open_store(path: impl AsRef<Path>) -> io::Result<Rc<dyn Store>> {
    match UnicodePstFile::open(path.as_ref()) {
        Ok(pst_file) => Ok(UnicodeStore::read(Rc::new(pst_file))?),
        Err(error) if is_ansi_version_error(&error) => {
            let pst_file = AnsiPstFile::open(path.as_ref())?;
            Ok(AnsiStore::read(Rc::new(pst_file))?)
        }
        Err(error) => Err(error),
    }
}

fn is_ansi_version_error(error: &io::Error) -> bool {
    error
        .get_ref()
